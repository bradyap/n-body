...
AXIS_MIN, AXIS_MAX = -800.0, 800.0  # Axis size of plot
FRAMES_BETWEEN_UPDATES = 400  # How many sim steps between plot updates
...
def run_sim(bodies, N, calc_func, threads):
    ...
    for step in range(NUM_STEPS):
        # Compute forces
        if threads > 1:
            calc_func(bodies, N, G, threads)
        else:
            calc_func(bodies, N, G)
...
def main():
    ...
    # Run simulation
    run_sim(bodies, N, nbody.compute_forces_serial, 1)
